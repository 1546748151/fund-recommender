...
### Task 8: OD-Driven Static Page Builder (REVISED)

> **设计变更**: PageBuilder 不再内联 HTML/CSS 字符串。改为读取 Open Design 设计稿作为模板，注入数据和 JS 逻辑后输出。

**Files:**
- Modify: `fund/page.py` (重写)
- Modify: `tests/test_page.py` (更新测试)
- Modify: `config.yaml` (新增 `od_template_dir`)

**Architecture:**
```
OD 设计稿 (index.html, detail.html)
    │
    ▼
PageBuilder._load_od_template(name)  ← 读取 HTML，提取样式
    │
    ▼
_inject_echarts_cdn + _inject_data_and_js  ← 注入 data.json + JS 渲染逻辑
    │
    ▼
output/index.html, detail.html, history.html
```
...
page:
  output_dir: "./output"
  top_n: 50
  od_template_dir: "D:/dev/open-design/.od/projects/4e53d8f3-4432-4ba0-a4e8-e2bb29af9f42"
```

- [ ] **Step 2: 更新测试，验证从 OD 模板读取**

重写 `tests/test_page.py`:
...
def test_build_uses_od_template(page_builder):
    page_builder.build()
...
def test_data_json_has_correct_structure(page_builder):
    page_builder.build()
...
```bash
cd C:/Users/郭鑫/fund-recommender && python -m pytest tests/test_page.py -v
```
Expected: FAIL — `AttributeError: 'PageBuilder' object has no attribute '_load_od_template'`

- [ ] **Step 4: 重写 PageBuilder（OD 模板驱动）**
...
class PageBuilder:
    ...
    def _inject_data_and_js(self, html: str, data: dict) -> str:
...
window.addEventListener('resize',()=>{{['chartTypeAvg','chartRiskReturn','chartTypeDist'].forEach(id=>{{const dom=document.getElementById(id);if(dom){{const inst=echarts.getInstanceByDom(dom);if(inst)inst.resize();}}}});}});
    ...
    def _write_index_html(self, data: dict):
        ...
        html = self._inject_data_and_js(html, data)
...
```bash
cd C:/Users/郭鑫/fund-recommender && python -m pytest tests/ -v
```
Expected: all 25 PASS

- [ ] **Step 7: 端到端验证 — 从真实 OD 模板生成并检查**

```bash
rm -f ./fund_data.db
python fund.py data update
python fund.py score run
python fund.py page build
python3 -c "
with open('output/index.html', encoding='utf-8') as f:
    html = f.read()
assert '--bg' in html
assert '--accent' in html
assert 'echarts' in html
assert 'tableBody' in html
print('OD template integration: OK')
"
```
...
```bash
git add config.yaml fund/page.py tests/test_page.py docs/superpowers/
git commit -m "refactor: switch PageBuilder to OD-driven templates instead of inline HTML"
```
